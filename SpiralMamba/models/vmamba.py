...
from models.spiral_scan import get_hui_path_origin_path
        ...
        if cascade2d:  # false
            ...
            if no_einsum:  # ture
                x_dbl = F.conv1d(xs.contiguous().view(B, -1, L), x_proj_weight.view(-1, D, 1),
                                 bias=(x_proj_bias.contiguous().view(-1) if x_proj_bias is not None else None), groups=K)
                dts, Bs, Cs = torch.split(x_dbl.view(B, K, -1, L), [R, N, N], dim=2)
                dts = F.conv1d(dts.contiguous().view(B, -1, L), dt_projs_weight.contiguous().view(K * D, -1, 1), groups=K)
            else:
                x_dbl = torch.einsum("b k d l, k c d -> b k c l", xs, x_proj_weight)
                if x_proj_bias is not None:
                    x_dbl = x_dbl + x_proj_bias.view(1, K, -1, 1)
...
class SS2D(nn.Module, mamba_init, SS2Dv0, SS2Dv2, SS2Dv2_spec, SS2Dv3):
    def __init__(
        self,
        mode='sptial',
        imgsize=7,
        ...
        **kwargs,
        ...
        super().__init__()
        kwargs.update(
            imgsize=imgsize, d_model=d_model, d_state=d_state, ssm_ratio=ssm_ratio, dt_rank=dt_rank,
...
class VSSBlock(nn.Module):
        ...
        **kwargs,
        ...
        if self.mlp_branch:
            if self.mode =='spectral':
                # dim = imgsize*imgsize
                # _MLP = Mlp if not gmlp else gMlp  # MLp
                # self.norm2 = nn.LayerNorm(dim)
                # mlp_hidden_dim = int(dim * mlp_ratio)
                # self.mlp = _MLP(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=mlp_act_layer,
                #                 drop=mlp_drop_rate, channels_first=False)
                self.mlc = MLC(input_channels=hidden_dim, ratio=1)
    ...
    def _forward(self, input: torch.Tensor, attn=None):
        x = input
        if self.ssm_branch: # true
            if self.post_norm:# false
                x = x + self.drop_path(self.norm(self.op(x)))
            else:
                if attn is not None:
                    x = x + self.drop_path(self.op(self.norm(x), attn))
                else:
                    x = x + self.drop_path(self.op(self.norm(x)))

        if self.mlp_branch:# true
            if self.post_norm:# false
                x = x + self.drop_path(self.norm2(self.mlp(x))) # FFN
            else:
                if self.mode == 'spectral':
                    x = x + self.drop_path(self.mlc(self.norm2(x)))  # FFN
                else:
                    x = x + self.drop_path(self.mlp(self.norm2(x))) # FFN

        return x

    def forward(self, input: torch.Tensor, attn=None):
        if self.use_checkpoint:
            return checkpoint.checkpoint(self._forward, input)
        else:
            return self._forward(input, attn)
...
class MLC(nn.Module):
    def __init__(self, input_channels, ratio=2):
        ...
        self.conv_layers = nn.Sequential(
            nn.Conv2d(in_channels=input_channels, out_channels=input_channels * ratio, kernel_size=(3, 3), padding=1),
            nn.ReLU(),
            nn.BatchNorm2d(input_channels * ratio),
            nn.Conv2d(in_channels=input_channels * ratio, out_channels=input_channels, kernel_size=(3, 3), padding=2, dilation=2),
            nn.ReLU(),
...
class RLB(nn.Module):
    def __init__(self, embed_dim=200, hidden_dim=64):
        super(RLB, self).__init__()
        self.LN = nn.LayerNorm(embed_dim * 2)
        self.conv1 = nn.Conv2d(in_channels=embed_dim * 2, out_channels=embed_dim, kernel_size=(3, 3), stride=1, padding=1)
        self.act = nn.GELU()
        self.conv2 = nn.Conv2d(in_channels=embed_dim, out_channels=1, kernel_size=(3, 3), stride=1, padding=1)
...
class OneMapFusion(nn.Module):
    def __init__(self, embed_dim=200, tao=0.8):
        super(OneMapFusion, self).__init__()
        self.tao = tao
        self.relationship = RLB(embed_dim=embed_dim, hidden_dim=64)

    def forward(self, x_spat, x_spec):
        x = torch.cat((x_spat, x_spec), dim=1)
        w_map = self.relationship(x)
        w_map = torch.sigmoid(w_map)

        ones = torch.ones_like(w_map)
        w_map = torch.where(w_map > self.tao, ones, w_map)
        tao1 = 1 - self.tao
        zores = torch.zeros_like(w_map)
        w_map = torch.where(w_map < tao1, zores, w_map)
        res = x_spat * w_map + x_spec * (1 - w_map)
        return res
...
class VSSM(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.fusion = OneMapFusion(embed_dim=dims, tao=tao)

        self.central_att = CenterAttention_orign(imgsize=imgsize)
    ...
    def _make_patch_embed_v2(in_chans=3, embed_dim=96, patch_size=4, patch_norm=True, norm_layer=nn.LayerNorm, channel_first=False):
        # if channel first, then Norm and Output are both channel_first
        stride = patch_size // 2 if patch_size > 1 else 1
        kernel_size = 1
        padding = 0
        return nn.Sequential(
    ...
    def _make_layer(
        ...
        **kwargs,
        ...
        return nn.Sequential(OrderedDict(
            ...
            blocks=blocks,
            blocks_spe=blocks_spe,
            downsample=downsample,
        ))

    def forward(self, x0: torch.Tensor):
        # att_orign = self.central_att(x0)
        ...
        for layer in self.layers:
            # x = layer(x)
            blocks = layer.blocks
            blocks_spe = layer.blocks_spe
            dsp = layer.downsample
            ...
            for blk_spe in blocks_spe:
                x_spe = blk_spe(x)

            x = self.fusion(x_spa, x_spe)
